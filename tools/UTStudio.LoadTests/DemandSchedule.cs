...
internal sealed class DemandSchedule(double? rate, long frequency = 0, bool detailed = true)
{
    ...
    internal DemandSnapshot Snapshot(long now)
    {
        lock (_gate)
        {
            ...
            return new(scheduled, _offered, Math.Max(0, scheduled - (_offered - _duplicates)), _duplicates,
                detailed ? (_offered == 0 ? 0 : _delaySum / _offered) : null, detailed ? _delayMax : null);
        }
    }
}
