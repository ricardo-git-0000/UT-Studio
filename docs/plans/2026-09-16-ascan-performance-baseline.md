...
2. Calibración Minimal/Full, piloto Release y primera baseline de la VM: completados. El overhead observado de Full quedó por debajo de la variabilidad entre repeticiones de la VM.
3. Pendiente: carga y soak prolongados, hardware/transporte reales y presupuestos específicos por transporte/configuración. Ninguna optimización queda seleccionada por este plan.
...
Hasta 100/s se conserva la fuente productiva con cadencia efectiva `production-fixed-delay`: después de producir y entregar cada frame espera un periodo completo, de modo que generación y backpressure se suman al periodo y pueden causar deriva. No administra slots ni implementa `skip-missed`; por tanto, `Demand.Missed` es cero y cualquier diferencia frente a la tasa objetivo se etiqueta como déficit diagnóstico, no como demanda omitida por el productor. `--pacing` selecciona únicamente algoritmos de la fuente experimental. Por encima de 100/s se usa fuente experimental, generador LCG y SamplePool propio, identificados en salida. No equivalen a coste de SyntheticRfGenerator ni al modo fixture/copia, que permanece pendiente.
...
La consola de progreso es periódica, no por frame; no calcula/copía histogramas en cada sondeo. La instrumentación conserva costes de timestamps, atomics, wrappers y locks. La calibración A/B Minimal/Full se completó antes del cierre de la baseline; el overhead observado quedó por debajo de la variabilidad entre repeticiones de esta VM. Esto no elimina la necesidad de repetirla si cambia la instrumentación o el entorno.
...
- Minimal conserva demanda programada/ofrecida/omitida de los algoritmos experimentales, generación/aceptación/consumo/liberación, rent/return y buffers pendientes, continuidad/pérdida funcional, progreso/watchdog, duraciones activa/cierre, resultado/errores/código de salida y snapshots iniciales/finales de CPU, GC y memoria. La validación productiva de continuidad permanece activa; producción no administra slots.
...
- Cabecera explícita: `telemetry=Minimal detailedPerFrameInstrumentation=disabled` o `telemetry=Full detailedPerFrameInstrumentation=enabled`. El esquema JSON vigente es 4: distingue `requestedPacing` y `effectivePacing` y usa null para métricas no recopiladas; consola usa unavailable. También son null/unavailable los histogramas Full sin observaciones. La lista de campos y semántica de extremos se documentan en el [README de LoadTests](../../tools/UTStudio.LoadTests/README.md).

Validación histórica de esa corrección: formato verificado; build Release sin advertencias/errores; 188 pruebas Core y 28 WPF correctas; dos smokes Minimal/Full experimentales con balances completos y cero buffers pendientes. Esos smokes verificaron comportamiento y no constituyeron por sí solos la baseline. La calibración comparativa posterior quedó completada y mostró un overhead inferior a la variabilidad observada entre repeticiones de la VM; después se cerró la primera baseline.
...
Las referencias productivas anteriores, de una sola ejecución por caso y por tanto no consolidadas estadísticamente, quedaron en 31,169–31,175/s para objetivos de 50/s y 60,992–61,084/s para objetivos de 100/s, con 2.048 y 65.535 muestras. Registraron cero errores, cero buffers pendientes y barrera confirmada. Los JSON históricos contabilizaron 6.782–6.789 y 14.028–14.093 slots como `Demand.Missed` mediante una rejilla diagnóstica externa; esa etiqueta queda superada porque la fuente productiva no gestiona slots. Las cifras representan cadencia observada `production-fixed-delay` y déficit diagnóstico frente al objetivo bajo VMware, no demanda realmente omitida, pérdida UT ni validación de `skip-missed`.
