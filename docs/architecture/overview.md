...
El [ADR 0007](../adr/0007-frame-source-ownership.md) concreta frame RF `ReadOnlyMemory<short>` y ownership exclusivo; el [ADR 0008](../adr/0008-latest-only-visual-delivery.md) concreta snapshots versionados, notificaciones fuera de locks, mailbox latest-only, máximo 1.024 puntos/30 Hz y métricas a 5 Hz. El [ADR 0010](../adr/0010-ascan-manual-cursor-measurements.md) añade medidas manuales aproximadas sobre el punto visual reducido más próximo, con estado neutral propiedad de Presentation. Los [ADR 0011](../adr/0011-shared-physical-time-viewport.md) y [ADR 0012](../adr/0012-shared-scan-time-navigation.md) definen un viewport temporal neutral, cerrable y compartible por grupos entre A-Scan y futuros B-Scan. Cada ViewModel expone una asociación local run/versión y los controles solo adoptan el viewport compatible con el snapshot admitido. La composición posee el coordinador compartido; una aplicación externa puede usar una instancia privada. Las operaciones largas admiten CancellationToken. No compartir un ChannelReader entre consumidores competidores: la futura distribución tendrá ramas separadas para procesamiento y almacenamiento sin pérdida silenciosa, visualización latest-only y métricas mediante muestreo. El ownership compartido se decidirá antes de introducir varios consumidores; el ownership inicial queda definido en ADR 0007. Véase [pipeline](data-pipeline.md).
